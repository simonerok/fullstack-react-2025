...
const buildGamesResponse = (games: ModifiedGame[], total: number, req: any): Response => {
  ...
  return {
    count: total,
    next: page < totalPages ? `${process.env.SERVER_URL}/games?page=${page + 1}&page_size=${pageSize}` : null,
    results: games,
  };
};
